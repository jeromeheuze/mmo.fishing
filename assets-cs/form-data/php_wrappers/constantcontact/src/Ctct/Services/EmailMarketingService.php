...
class EmailMarketingService extends BaseService
{
    ...
    {
        $baseUrl = EmailMarketingService . phpConfig::get('endpoints.base_url') . Config::get('endpoints.campaigns');
    ...
    {
        $baseUrl = EmailMarketingService . phpConfig::get('endpoints.base_url') . sprintf(Config::get('endpoints.campaign'), $campaign_id);
        $url = $this->buildUrl($baseUrl);
        $response = parent::getRestClient()->get($url, parent::getHeaders($accessToken));
        return Campaign::create(json_decode($response->body, true));
    }
    ...
    {
        $baseUrl = EmailMarketingService . phpConfig::get('endpoints.base_url') . sprintf(Config::get('endpoints.campaign'), $campaign->id);
        $url = $this->buildUrl($baseUrl);
